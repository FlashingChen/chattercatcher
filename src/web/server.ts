...
<body class="gradient-bg">
  ...
  <main class="main-content">
    <div class="view active" id="view-overview">
      <div class="page-header">
        ...
        <p class="page-subtitle">本地优先的家庭群知识库 · 问答必须先检索 RAG 证据</p>
      </div>
  ...
  <script>
    ...
    async function processNow() {
      ...
      try {
        var response = await fetch("/api/process/messages", {
          method: "POST",
          headers: { "x-chattercatcher-web-token": webActionToken }
        });
        var result = await response.json();
        if (!response.ok) { showToast(result.message || "\u5904\u7406\u5931\u8d25", "error"); return; }
    ...
    document.addEventListener("click", async function(event) {
      ...
      try {
        var response = await fetch("/api/cron-jobs/" + encodeURIComponent(id), {
          method: "DELETE",
          headers: { "x-chattercatcher-web-token": webActionToken }
        });
        var result = await response.json();
